...
def render_markdown(data: dict[str, object]) -> str:
    ...
    if _JINJA_ENV is not None and _JINJA_ENV.loader is not None:
        try:
            ...
            LOGGER.exception(
                "Markdown template rendering failed; using fallback renderer."
            )
...
def _build_markdown_context(data: dict[str, object]) -> dict[str, Any]:
    ...
    safe_findings = findings if isinstance(findings, Sequence) else []
    context = {
        ...
        "recommendations": _build_recommendations(safe_findings),
