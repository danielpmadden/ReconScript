...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--target", required=True, help="Target IP address or hostname within scope"
    )
    parser.add_argument(
        "--expected-ip", help="Require hostnames to resolve to this IP address"
    )
    ...
    parser.add_argument(
        "--consent-file", type=Path, help="Path to signed scope manifest JSON"
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--outfile", type=Path, help="Explicit output path for rendered report"
    )
    parser.add_argument(
        "--sign-report",
        action="store_true",
        help="Sign the report hash using the local signing key",
    )
    parser.add_argument(
        "--log-json",
        action="store_true",
        help="Emit structured JSON logs to the log file",
    )
    parser.add_argument(
        "--log-file", type=Path, help="Write logs to the specified path"
    )
    parser.add_argument(
        "--enable-ipv6",
        action="store_true",
        help="Resolve IPv6 addresses where available",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Generate a report skeleton without network activity",
    )
    parser.add_argument(
        "--extra-redact",
        action="append",
        default=[],
        help="Additional header names to redact",
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    persisted = persist_report(
        report, consent_source=consent_source, sign=args.sign_report
    )
